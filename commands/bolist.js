module.exports = {
	Execute: (Args, message, _, serverdata) => {
		DiscordCards.classHandler.classes.Market.getAllBuy().then((offers) => {
			DiscordCards.classHandler.classes.Card.getAll().then((items) => {
				...
				DiscordCards.classHandler.classes.Series.getAll().then((series) => {
					...
					k.map((a, b) => {
						...
						itmz.push(`< ${total}x ${y[item].name} (${c.buy.price.formatNumber()} ${_("_dollar")} ${_('each_lowercase')}) {ID: ${c.id}} [${bought.formatNumber()} ${_('bought_lowercase')}] >`);
					});
					...
					let sendList = function(pp){
						let head = `#===== [${_('gm_bo')} (${_('page')} ${p.formatNumber()}/${Object.keys(sets).length.formatNumber()})] =====#`;
						let list = "```md\n"+head+"\n";
	...
	Description: "Shows a list of all buying offers",
	Usage: "[page]",
	Cooldown: 2,
	Category: "Market",
	Aliases: ["gblist", "gbl", "bol"]
}