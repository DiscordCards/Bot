	...
	Execute: (Args, message, _, serverdata) => {
		if(Args.length >= 1){
			...
			DiscordCards.classHandler.classes.Club.getByName(name).then((club) => {
				...
				Promise.all(club.invites.map(u=>rdb.r.table("users").get(u).run(rdb.conn))).then(members => {
					let y = members;
					let p = 1;
					let ipp = 15;
					...
					let sendList = function(pp){
						let head = `!===== [${_('club_header_invites', {club:club.name})} (${_('page')} ${p.formatNumber()}/${Object.keys(sets).length.formatNumber()})] =====!`;
						let list = "```diff\n"+head+"\n";
