module.exports = {
				...
				DiscordCards.classHandler.classes.Series.getAll().then((series) => {
					...
					Object.keys(y).map((a) => {
						if(!found && (item.toLowerCase() === y[a].name.toLowerCase() || (isId && item.slice(1) === y[a].id))){
							found = true;
							DiscordCards.classHandler.classes.Market.getAllSell().then((trades) => {
								let rows = [];
								...
								for(let trade of trades){
									...
									if(sold !== total){
										if(item === a){
											rows.push(`< ${total}x ${y[item].name} (${trade.sell.price.formatNumber()} ${_("_dollar")} ${_('each_lowercase')}) {ID: ${trade.id}} [${sold.formatNumber()} ${_('sold_lowercase')}] >`);
										}
									}
								}
								...
								let sendList = function(pp){
									let head = `#===== [${_('gm_so')} ${_('gm_based_search')} (${_('page')} ${p.formatNumber()}/${Math.ceil(rows.length/ipp).formatNumber()})] =====#`;
									let list = "```md\n"+head+"\n";
							...
							DiscordCards.classHandler.classes.Market.getAllSell().then((trades) => {
								...
								let sendList = function(pp){
									let head = `#===== [${_('gm_so')} ${_('gm_based_rarity', {rarity:rarityName})} (${_('page')} ${p.formatNumber()}/${Math.ceil(rows.length/ipp).formatNumber()})] =====#`;
									let list = "```md\n"+head+"\n";
									list += sets['set'+pp].join("\n");
									list += "\n"+head+"```";
									return list;
							...
							DiscordCards.classHandler.classes.Market.getAllSell().then((trades) => {
								...
								let sendList = function(pp){
									let head = `#===== [${_('gm_so')} ${_('gm_based_series', {series:seriesName})} (${_('page')} ${p.formatNumber()}/${Math.ceil(rows.length/ipp).formatNumber()})] =====#`;
									let list = "```md\n"+head+"\n";
									list += sets['set'+pp].join("\n");
									list += "\n"+head+"```";
									return list;
	...
	Description: "Searches for an item on selling offers.",
	Usage: "[item] [page]",
	Extra: {
