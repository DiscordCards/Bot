module.exports = {
		...
		User.get().then(user => {
			...
			if(Args.length >= 2){
				DiscordCards.classHandler.classes.Club.getByName(Args.slice(1).join(" ")).then((club) => {
					if(!club){
						message.channel.send(`Couldn't find that club!`);
						return;
					}
					...
					if(cm.exists(message.author)){
						if(cm.isAdmin(message.author)){
							...
							if(!club.bans.includes(usr.id)){
								message.channel.send(_('not_banned'));
								return;
							}

							if(usr !== null){
								Club.unban(usr.id).then(()=>{
									message.channel.send(_("club_unban", {user:usr.username}));
	...
	Description: "Unbans someone from a club.",
	Usage: "[@mention] [name]",
	Cooldown: 1,
	Category: "Clubs"
}